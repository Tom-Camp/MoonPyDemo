...
class MoonPy:
    ...
    def _caclulate_position_in_cycle(self, phase_date=None):
        ...
        position = (date_diff % self.period_in_seconds) / self.period_in_seconds
        if position < 0:
            position = 1 + position
    ...
    def _get_base_full_moon(self):
        ...
        known_full_moon = datetime(2008, 12, 12, 16, 37)
    ...
    def _set_phase_id(self):
        ...
        if self.position >= 0.474 and self.position <= 0.53:
            ...
            self.phase_id = 3
        elif self.position >= 0.975 or self.position <= 0.026:
            self.phase_id = 4
